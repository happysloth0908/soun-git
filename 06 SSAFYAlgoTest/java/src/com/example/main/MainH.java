...
// ----- 여기에 코드 작성
			...
			for (int d = 0; d < 4; d++) {
				...
				if (nr < N || nc < N || nr >= 0 || nc >= 0 || mapData[nr][nc].equals("H")) {
					switch (d) {
					case 0:
						output = "U S";
						break;
	...
	private static int[] bfs(int x, int y) {
		...
		while (!qu.isEmpty()) {
			...
			for (int d = 0; d < 4; d++) {
				...
				//mapData[i][j].equals("A")
				if (mapData[nr][nc].equals("R") || mapData[nr][nc].equals("W") || mapData[nr][nc].equals("T") || mapData[nr][nc].equals("H") ) {
